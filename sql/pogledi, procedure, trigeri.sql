...
-- -----------------------
delimiter $$
create procedure `dodaj_fakturu`(in pBrojFakture int, in pIdProizvodjac int, in pIdZaposlenog int)
begin
	insert into faktura (IdFaktura, BrojFakture, IdProizvodjac, IdZaposlenog) values (null, pBrojFakture, pIdProizvodjac, pIdZaposlenog);
end$$
delimiter ;
...
begin
	...
	insert into kozmetika (JedinicaMjere, Barkod) values (pJedinicaMjere, pBarkod);
...
-- -----------------------
delimiter $$ 
create procedure dodaj_medicinski_aparat(in pBarkod int, in pNazivArtikla varchar(50), in pNabavnaCijena decimal(5,2), in pProdajnaCijena decimal(5,2), in pIdProizvodjac int, in pIdTipArtikla int)
begin
	insert into artikal (Barkod, NazivArtikla, IdProizvodjac) values (pBarkod, pNazivArtikla, pIdProizvodjac);
	insert into medicinski_aparat(Barkod) values (pBarkod);
    insert into artikal_tip_artikla (Barkod, IdTipArtikla) values (pBarkod, pIdTipArtikla);
    insert into cijena (NabavnaCijena, ProdajnaCijena, Barkod, IdTipArtikla) values (pNabavnaCijena, pProdajnaCijena, pBarkod, pIdTipArtikla);
end$$
delimiter ;
...
-- -----------------------
delimiter $$
create procedure dodaj_racun(in pIdZaposlenog int(11))
begin
	insert into racun(IdRacuna, IdZaposlenog) values (null, pIdZaposlenog);
end$$
delimiter ;
...
-- -----------------------
delimiter $$
create procedure dodaj_zaposlenog(in pIme varchar(45), in pPrezime varchar(45), in pJMBG char(13), in pEmail varchar(50), in pDatumZaposlenja date, in pPlata double,
in pAdresaStanovanja varchar(50), in pRadnoVrijeme varchar(20), in pBrojPoste int, in pIdApoteke int)
begin
    insert into zaposleni(IdZaposlenog, Ime, Prezime, JMBG, Email, DatumZaposlenja, Plata, AdresaStanovanja, RadnoVrijeme, BrojPoste, IdApoteke) values (pIme, pPrezime, pJMBG, pEmail, pDatumZaposlenja, pPlata, pAdresaStanovanja, pRadnoVrijeme, pBrojPoste, pIdApoteke);
end$$
delimiter ;
